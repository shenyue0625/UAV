...
class Ordering extends Component {
  ...
  getPrice = (data) => {
    ...
    function dist(data){
         ...
         if(data.Ordering.deliveryMethod === drone){
             ...
             return getRoute(receiverAddress,senderLatAddress)// 这里应该是 {google api里 best route的测算公式}
         }
    }    
  ...
  checkPriceOnClick = data => {
    this.setState({
      price: {getPrice(data)}, //$30只是为了测试写的hard code。NEED TO BE CHANGED TO CALCULATED PRICE，并且把price负值给Ordering.price
      Ordering: data,
    });
  };
